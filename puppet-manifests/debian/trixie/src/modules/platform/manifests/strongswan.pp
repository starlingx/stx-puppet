...
class platform::strongswan::config
  ...
  $strongswan_override_dir = '/etc/systemd/system/strongswan.service.d'
  $pmon_config_file = '/etc/pmon.d/strongswan.conf'

  # Restart charon
  if (find_file($pmon_config_file)) {
    $ipsec_restart_cmd = '/usr/local/sbin/pmon-restart charon-systemd'
  } else {
    $ipsec_restart_cmd = '/usr/bin/systemctl restart strongswan'
  }
  ...
  # Add strongswan service override
  ...
  # Reload systemd
  -> exec { 'perform systemctl daemon reload for strongswan override':
  ...
  -> file { '/tmp/strongswan.conf':
    ensure  => file,
    content => template('platform/strongswan-pmond-conf.erb'),
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
  }
  ...
  -> exec { 'Move pmon config file to its location':
    command => "/usr/bin/mv /tmp/strongswan.conf ${pmon_config_file}",
  }
