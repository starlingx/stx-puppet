...
class platform::filesystem::backup::params (
  $lv_name = 'backup-lv',
  $lv_size = '1',
  $mountpoint = $facts['os']['family'] ? { 'Debian' => '/var/rootdirs/opt/backups', default => '/opt/backups' },
...
class platform::filesystem::conversion::params (
  ...
  $mountpoint = $facts['os']['family'] ? { 'Debian' => '/var/rootdirs/opt/conversion', default => '/opt/conversion' },
